...
<div class="form-row">
    <div class="col-6 col-md-6 mb-3">
        ...
        <select id="document_type" data-toggle="select" name="document_type"
            ...
            <option value="national_id" {{ $user->document_type == 'national_id' ? 'selected' : 'true'}} {{ old('document_type') == '' ? 'selected' : 'true'}}>National ID</option>
            <option value="passport" {{ $user->document_type == 'passport' ? 'selected' : 'true'}}>Passport</option>
        </select>
        @error('document_type')
        <div class="invalid-feedback"
             style="display: block !important;">{{ $message }}</div>
    ...
    <div class="col-6 col-md-6 mb-3">
        ...
        <select id="nationality" data-toggle="select" name="nationality"
            ...
            @foreach($nationalities as $nationality)
                <option value="{{ $nationality }}" {{ $user->nationality == $nationality ? 'selected' : ''}}@if($user->nationality =='') {{ $nationality == 'Malaysian' ? 'selected' : ''}} @endif >
                    {{ $nationality }}
                </option>
            @endforeach
        </select>
        ...
        <div class="row">
            <div class="col-sm-8">
                <input type="text" class="form-control" disabled name="national_passport_file_link_name"
                       id="national_passport_file_link_name" value="{{ $user->getFileName()  }}" style="background:white;">
            </div>
            <div class="col-sm-2" style="padding-left: 0">
                ...
                <label class="national_passport_file_link_name_label" style="padding-top: 5px;"></label>
            </div>
        </div>
...
<div class="form-row">
    ...
    <div class="col-6 col-md-6 mb-3">
        ...
        <input type="text" id="mobile_number" name="mobile_number"
               ...
               value="{{ old('mobile_number') != null ? old('mobile_number') : $user->mobile_number }}" maxlength="13" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');">
        @error('mobile_number')
        <div class="invalid-feedback"
             style="display: block !important;">{{ $message }}</div>
        @enderror
